...
<html>
 <head>
    ...
    <div class="left">
        ...
        |
        <a href="/api/index.php?s=/Admin/Comment/logout">退出</a>
    </div>
    <div class="right">
	...
	<div id="content" class="white">
        ...
        <div class="bloc">
		    <div class="title"><?php if(is_array($topCates)): $i = 0; $__LIST__ = $topCates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="/api/index.php?s=/Admin/Comment/index/tid/<?php echo ($tid); ?>/cid/{vo.id}"><?php echo ($vo["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?></div>
		    <div class="content"> 
				...
				<table>
		            ...
		            <tbody>
		            	<?php if(is_array($commentlist)): $i = 0; $__LIST__ = $commentlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
		                    ...
		                    <td class="picture" style="width:100px;"><a href="<?php echo ($vo["thumbpic"]); ?>" class="zoombox"><img src="<?php echo ($vo["thumbpic"]); ?>" class="thumb" alt="<?php echo ($vo["username"]); ?>" /></a></td>
		                    <td><?php echo ($vo["username"]); ?></td>
		                    <td><?php echo ($vo["theme"]); ?></td>
		                    <td><?php echo ($vo["content"]); ?></td>
		                    <td><?php echo ($vo["agreenum"]); ?></td>
		                    <td><?php if($vo['is_checked'] == 0): ?><a href="javascript:;" onclick="checkComment(<?php echo ($vo["id"]); ?>,this);">未审核</a><?php else: ?><a href="javascript:;" onclick="checkComment(<?php echo ($vo["id"]); ?>,this);">已通过</a><?php endif; ?></td>
		                    <td><?php echo (date('Y-m-d',$vo["date"])); ?></td>
		                    <td style="text-align:center;"><a href="/api/index.php?s=/Admin/Comment/del/id/<?php echo ($vo["id"]); ?>" title="Delete this content" onclick="return confirm('你确定要删除这条评论吗？');"><img src="/api/Public/Admin/img/icons/actions/delete.png" alt="删除" /></a></td>
	...
	<script type="text/javascript">
		function checkComment(id,target){
			var $target=$(target);
			$.ajax({
				url:'/api/index.php?s=/Admin/Comment/checkComment',
				data:{"id":id},
				type:'post',
				dataType:'json',
				success:function(response){
					if(response.errno == 0){
						if(response.is_checked == 1){
							alert('审核通过！');
							$target.parent().html('<a href="javascript:;" onclick="checkComment('+id+',this);">已通过</a>');
						}else{
							alert('审核失败！');
							$target.parent().html('<a href="javascript:;" onclick="checkComment('+id+',this);">未审核</a>');
						}
					}
				}
			});
