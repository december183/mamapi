...
<html>
 <head>
	...
	<div id="content" class="white">
        <h1><img src="/api/Public/Admin/img/icons/posts.png" alt="" /> 用户组管理-添加用户组</h1>
        <div class="bloc">
		    <div class="title"><a href="/api/index.php?s=/Admin/Group/index">用户组列表</a> &gt; <a href="/api/index.php?s=/Admin/Group/add">添加用户组</a></div>
		    <form method="post" action="">
		    <div class="content">
	    		<div class="input">用户组名：<input type="text" name="groupname" /> <em class="red">*</em></div>
	    		<div class="input">
	    			权限设置：
	    			<div class="pad-left">
		    			<?php if(is_array($authlist)): $i = 0; $__LIST__ = $authlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label><input type="checkbox" name="auth[]" value="<?php echo ($vo["id"]); ?>" /><?php echo ($vo["name"]); ?></label><br/>
		    			<?php if(is_array($vo['child'])): $i = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><label><input type="checkbox" name="auth[]" value="<?php echo ($voo["id"]); ?>" /><?php echo ($voo["name"]); ?></label>　<?php endforeach; endif; else: echo "" ;endif; ?><br/><?php endforeach; endif; else: echo "" ;endif; ?>
	    			</div>
	    		</div>
		        <div class="input">
		        	是否启用：
