...
<html>
 <head>
    ...
    <div class="left">
        ...
        |
        <a href="/api/index.php?s=/Admin/Event/logout">退出</a>
    </div>
    <div class="right">
	...
	<div id="content" class="white">
        <h1><img src="/api/Public/Admin/img/icons/posts.png" alt="" /> 活动管理-活动列表</h1>
        <div class="bloc">
		    <div class="title"><?php if(is_array($topCates)): $i = 0; $__LIST__ = $topCates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="/api/index.php?s=/Admin/Event/index/cid/<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></a><?php if($i != count($topCates)): ?>&nbsp;&gt;&nbsp;<?php endif; endforeach; endif; else: echo "" ;endif; ?></div>
		    <div class="content">
				...
				<table>
		            ...
		            <tbody>
		            	<?php if(is_array($eventlist)): $i = 0; $__LIST__ = $eventlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
		                    ...
		                    <td style="text-align:center;"><a href="/api/index.php?s=/Admin/Event/del/cid/<?php echo ($cid); ?>/id/<?php echo ($vo["id"]); ?>" title="Delete this content" onclick="return confirm('你确定要删除这个活动吗？');"><img src="/api/Public/Admin/img/icons/actions/delete.png" alt="删除" /></a></td>
	...
	<script type="text/javascript">
		function setStatus(id,target){
			var $target=$(target);
			$.ajax({
				url:'/api/index.php?s=/Admin/Event/setStatus',
				data:{"id":id},
				type:'post',
				dataType:'json',
		...
		function isRec(id,target){
			var $target=$(target);
			$.ajax({
				url:'/api/index.php?s=/Admin/Event/isRec',
				data:{"id":id},
				type:'post',
				dataType:'json',
				success:function(response){
