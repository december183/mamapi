...
<html>
 <head>
	...
	<div id="content" class="white">
        <h1><img src="/api/Public/Admin/img/icons/posts.png" alt="" /> 菜单管理-修改菜单</h1>
        <div class="bloc">
		    <div class="title"><a href="/api/index.php?s=/Admin/Auth/index">菜单列表</a> &gt; <a href="/api/index.php?s=/Admin/Auth/add">添加菜单</a> &gt; <a href="javascript:;">修改菜单</a></div>
		    <form method="post" action="">
		    <input type="hidden" name="id" value="<?php echo ($oneAuth["id"]); ?>" />
		    <div class="content">
	    		<div class="input">菜单名称：<input type="text" name="name" value="<?php echo ($oneAuth["name"]); ?>" /> <em class="red">*</em></div>
	    		<div class="input">图标样式：<input type="text" name="icon" value="<?php echo ($oneAuth["icon"]); ?>" /> <em class="red">*</em></div>
	    		<div class="input">菜单链接：<input type="text" name="url" value="<?php echo ($oneAuth["url"]); ?>" /> <em class="red">*</em></div>
	    		<div class="input">
		            上级节点：
		            <select name="pid" id="select">
		            <option value="0">顶级菜单</option>
		            <?php if(is_array($authlist)): $i = 0; $__LIST__ = $authlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($oneAuth['pid'] == $vo['id']): ?>selected="selected"<?php endif; ?>><?php echo ($vo["name"]); ?></option>
		                <?php if(is_array($vo['child'])): $i = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($voo["id"]); ?>" style="padding-left:25px;" <?php if($oneAuth['pid'] == $voo['id']): ?>selected="selected"<?php endif; ?>><?php echo ($voo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
		            </select> <em class="red">*</em>
		        </div>
		        <div class="input">
		        	是否显示：
		            <input type="radio" id="radio1" name="status" value="1" <?php if($oneAuth['status'] == 1): ?>checked="checked"<?php endif; ?> /><label for="radio1" class="inline">是</label>
		            <input type="radio" id="radio2"  name="status" value="0" <?php if($oneAuth['status'] == 0): ?>checked="checked"<?php endif; ?> /><label for="radio2" class="inline">否</label>
		        </div>
		        <div class="submit">
