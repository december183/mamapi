...
<html>
 <head>
	...
	<div id="content" class="white">
        <h1><img src="/api/Public/Admin/img/icons/posts.png" alt="" /> 品牌管理-修改品牌</h1>
        <div class="bloc">
		    <div class="title"><?php if(is_array($topCates)): $i = 0; $__LIST__ = $topCates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="/api/index.php?s=/Admin/Brand/index/cid/<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></a>&nbsp;&gt;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?><a href="/api/index.php?s=/Admin/Brand/add">添加品牌</a> &gt; <a href="/api/index.php?s=/Admin/Brand/edit/id/<?php echo ($oneBrand["id"]); ?>">修改品牌</a></div>
		    <form method="post" action="" enctype="multipart/form-data">
		    <input type="hidden" name="id" value="<?php echo ($oneBrand["id"]); ?>" />
		    <div class="content">
	    		<div class="input">品牌标题：<input type="text" name="name" value="<?php echo ($oneBrand["name"]); ?>" /> <em class="red">*</em></div>
	    		<div class="input">
		            品牌栏目：
		            <select name="cateid" id="cateid">
		            <option value="0">--请选择品牌栏目--</option>
		            <?php if(is_array($topCates)): $i = 0; $__LIST__ = $topCates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($oneBrand['cateid'] == $vo['id']): ?>selected="selected"<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		            </select> <em class="red">*</em>
		        </div>
				<div class="input">
		            logo　图：<input type="file" name="pic" /> <img src="<?php echo ($oneBrand["logo"]); ?>" class="thumb" alt="<?php echo ($oneBrand["name"]); ?>" /> <em class="red">*</em>
		        </div>
		        <div class="input textarea">
		            <span class="middle">品牌简介：</span><textarea name="descript" rows="3" cols="5"><?php echo ($oneBrand["descript"]); ?></textarea>
		        </div>
		        <div class="submit">
